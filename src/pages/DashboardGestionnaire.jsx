...
const TrashIcon = ({ size = 14 }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" aria-hidden="true" style={{ flexShrink: 0 }}>
    <path d="M4 7h16M9 7V5a1 1 0 011-1h4a1 1 0 011 1v2m1 0v12a1 1 0 01-1 1H8a1 1 0 01-1-1V7" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
);
const MenuIcon = () => (
...
const GearIcon = ({ size = 14 }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" aria-hidden="true" style={{ flexShrink: 0, marginRight: 5, verticalAlign: '-2px' }}>
    ...
    <div style={s.card}>
      ...
      {showForm && (
        <form onSubmit={handleInvite} style={s.formPanel}>
          {!selectedResidence ? (
            <p style={{ margin: 0, color: '#b45309', fontSize: 13, fontWeight: '500' }}>
      ...
      {success && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#dcfce7', border: '1px solid #bbf7d0', borderRadius: 8, padding: '10px 14px', marginBottom: 16 }}>
          ...
          <p style={{ margin: 0, color: '#15803d', fontSize: 13, fontWeight: '500' }}>{success}</p>
        </div>
      )}
        ...
        <div style={s.tableWrapper}>
          <table style={s.table}>
            ...
            <tbody>
              {residents.map(r => (
                <tr key={r.id}>
                  <td style={{ ...s.td, color: '#0f172a', fontWeight: '600' }}>{r.nom}</td>
        ...
        <div style={s.tableWrapper}>
          <table style={s.table}>
            ...
            <tbody>
              {paiements.map(p => (
                <tr key={p.id}>
                  <td style={{ ...s.td, color: '#0f172a', fontWeight: '600' }}>{p.resident?.nom ?? p.resident_nom ?? '—'}</td>
                  ...
                  <td style={s.td}>
                    {p.justificatif_url && (
                      <a href={p.justificatif_url} target="_blank" rel="noreferrer" style={{ color: '#0c9576', fontSize: 13, fontWeight: '600', marginRight: 8, textDecoration: 'none' }}>Voir justificatif</a>
                    )}
                    <button
            ...
            <tbody>
              {problemes.map(p => (
                <tr key={p.id}>
                  ...
                  <td style={s.td}>
                    {p.photo_url
                      ? <a href={p.photo_url} target="_blank" rel="noreferrer" style={{ color: '#0c9576', fontSize: 13, fontWeight: '600', textDecoration: 'none' }}>Voir photo</a>
        ...
        <div style={s.tableWrapper}>
          <table style={s.table}>
            ...
            <tbody>
              {annonces.map(a => (
                <tr key={a.id}>
                  <td style={{ ...s.td, color: '#0f172a', fontWeight: '600' }}>{a.titre}</td>
      ...
      {!loading && (
        isMobile ? (
          selectedId ? (
            <div style={{ display: 'flex', flexDirection: 'column', height: 500 }}>
              <button
                ...
                style={{ ...s.btn, background: '#f1f5f9', color: '#475569', border: '1px solid #e2e8f0', marginBottom: 12, alignSelf: 'flex-start' }}
              >
                ← Retour
              </button>
                  ...
                  return (
                    <div key={m.id} style={{ display: 'flex', justifyContent: mine ? 'flex-end' : 'flex-start', marginBottom: 10 }}>
                      <div style={{ maxWidth: '80%', ...(mine ? bubbleMine : bubbleOther), padding: '9px 13px', fontSize: 14 }}>
                        {!mine && <div style={{ fontSize: 11, fontWeight: '600', marginBottom: 3, color: '#0c7860' }}>{m.expediteur_nom ?? '—'}</div>}
              ...
              {showNewBadge && (
                <div style={{ textAlign: 'center', padding: '4px 0' }}>
                  <button onClick={scrollToBottom} style={{ background: 'linear-gradient(135deg, #14b890, #0c9576)', color: '#fff', border: 'none', borderRadius: 20, padding: '6px 18px', fontSize: 13, cursor: 'pointer', boxShadow: '0 4px 14px rgba(20,184,144,0.35)', fontFamily: 'inherit' }}>Nouveau message ↓</button>
                </div>
              )}
              <form onSubmit={handleSend} style={{ display: 'flex', gap: 8, paddingTop: 10, borderTop: '1px solid #e2e8f0' }}>
            ...
            <div>
              ...
              {conversations.map(c => (
                <div key={c.id} onClick={() => setSelectedId(c.id)} style={{ padding: '13px 10px', borderBottom: '1px solid #eef2f7', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderRadius: 8 }}>
                  <div style={{ overflow: 'hidden', flex: 1 }}>
                    <div style={{ fontWeight: c.unread > 0 ? '700' : '500', fontSize: 15, color: c.unread > 0 ? '#0f172a' : '#475569' }}>{c.nom}</div>
                    <div style={{ fontSize: 13, color: '#64748b', marginTop: 3 }}>{c.last ? (c.last.contenu.slice(0, 40) + (c.last.contenu.length > 40 ? '…' : '')) : 'Aucun message'}</div>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
                    {c.unread > 0 && <span style={{ background: '#14b890', color: '#fff', borderRadius: 10, padding: '2px 8px', fontSize: 12, fontWeight: '700' }}>{c.unread}</span>}
                    <button
                      onClick={e => handleDeleteConversation(e, c.id, c.nom)}
                      style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: '#fee2e2', color: '#b91c1c', border: '1px solid #fecaca', borderRadius: 6, padding: '4px 8px', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }}
                    ><TrashIcon /> Supprimer</button>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div style={{ display: 'flex', gap: 0, height: 460 }}>
            <div style={{ width: 230, borderRight: '1px solid #e2e8f0', overflowY: 'auto', paddingRight: 0, flexShrink: 0 }}>
              {conversations.length === 0 && <p style={s.empty}>Aucun message.</p>}
              {conversations.map(c => (
                <div key={c.id} onClick={() => setSelectedId(c.id)} style={{ padding: '11px 12px', borderRadius: 8, cursor: 'pointer', marginBottom: 2, background: selectedId === c.id ? '#ecfdf7' : 'transparent', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderLeft: selectedId === c.id ? '2px solid #14b890' : '2px solid transparent' }}>
                  <div style={{ overflow: 'hidden', flex: 1 }}>
                    <div style={{ fontWeight: c.unread > 0 ? '700' : '500', fontSize: 14, color: c.unread > 0 ? '#0f172a' : '#475569' }}>{c.nom}</div>
                    <div style={{ fontSize: 12, color: '#64748b', marginTop: 2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{c.last ? (c.last.contenu.slice(0, 26) + (c.last.contenu.length > 26 ? '…' : '')) : 'Aucun message'}</div>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0, marginLeft: 6 }}>
                    {c.unread > 0 && <span style={{ background: '#14b890', color: '#fff', borderRadius: 10, padding: '1px 7px', fontSize: 11, fontWeight: '700' }}>{c.unread}</span>}
                    <button
                      onClick={e => handleDeleteConversation(e, c.id, c.nom)}
                      title={`Supprimer la conversation avec ${c.nom}`}
                      aria-label={`Supprimer la conversation avec ${c.nom}`}
                      style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', background: '#fee2e2', color: '#b91c1c', border: '1px solid #fecaca', borderRadius: 5, padding: '4px 6px', fontSize: 12, cursor: 'pointer', lineHeight: 1, fontFamily: 'inherit' }}
                    ><TrashIcon /></button>
                  </div>
                </div>
              ))}
    ...
    <div style={s.page}>
      ...
      <nav style={s.nav}>
        ...
        <select
          ...
          style={{
            background: '#fff',
            color: residences.length === 0 ? '#94a3b8' : '#0f172a',
          ...
          <div style={s.mobileMenu}>
            {TABS.map(tab => (
              ...
              >
                ...
                {tab.label}
                {tab.key === 'messages' && unreadCount > 0 && (
                  <span style={{ background: '#14b890', color: '#fff', borderRadius: 10, padding: '1px 6px', fontSize: 11, fontWeight: '700', marginLeft: 8 }}>{unreadCount}</span>
                )}
                {tab.key === 'demandes' && demandesCount > 0 && (
